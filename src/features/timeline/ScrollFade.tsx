...
export function ScrollFade({ children, horizontal = false, label }: {
  children: ReactNode; horizontal?: boolean; label: string;
}) {
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      const remaining = horizontal ? element.scrollWidth - element.clientWidth - start
  ...
  return <div ref={ref} className={`preview-scroll${horizontal ? ' preview-scroll-horizontal' : ''}`}
