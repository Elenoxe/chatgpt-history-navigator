...
export function PreviewFile({ target, src, name, image = false, mimeType = "" }: PreviewFileProps) {
  ...
  const scopedTarget = target && {
    ...
    sharedId: target.sharedId ?? context.sharedId,
  ...
  useEffect(() => {
    if (query.error)
      console.error("[chatgpt-history-navigator] Failed to resolve preview file:", query.error)
  }, [query.error])
  return (
    <span className="preview-attachment">
      {failed ? (
        url ? (
        ...
        mimeType.startsWith("audio/") ? (
          <audio controls preload="none" src={url} onError={fail} />
