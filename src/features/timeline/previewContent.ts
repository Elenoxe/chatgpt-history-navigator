...
export function getWritingReferences(message: ConversationMessage, content: string, scopedReferences: unknown) {
  ...
  return records(message.metadata.content_references).flatMap(reference => {
    ...
    if (!marker || typeof start !== 'number' || typeof end !== 'number' ||
        original.slice(start, end) !== marker) return [];
    const matches: Record<string, unknown>[] = [];
    for (let offset = content.indexOf(marker); offset >= 0; offset = content.indexOf(marker, offset + marker.length)) {
...
export function parsePreviewReference(reference: Record<string, unknown>, sourceLabel: string): PreviewReferenceData {
  const items = [reference, ...records(reference.items)].flatMap(item => {
    const file = record(item.fileItem);
    return [{ ...item, ...file }];
  });
  const label = [...new Set(items.map(item => text(item.title) || text(item.filename) || text(item.name)).filter(Boolean))].join(' · ') || text(reference.alt) || sourceLabel;
  const links = items.flatMap(item => {
    const href = previewUrl(item.cloud_doc_url) || previewUrl(record(item.extra).cloud_doc_url) || previewUrl(item.internalHref) || previewUrl(item.url);
  ...
  const files = items.flatMap(item => {
    ...
    const isFileCitation = record(item.fff_metadata).file_reference_type === 'file_citation' || reference.type === 'file' || /^(?:filecite|\[citefilecite)/i.test(text(reference.matched_text).trim());
    const id = isFileCitation ? text(item.id) : '';
    const mountedLibraryId = text(item.mounted_library_file_id) ||
      [libraryId, id].find(value => /^(?:file-library:)?external-/.test(value));
    const target: PreviewFileTarget | undefined = fileId ? { fileId } : mountedLibraryId
      ? { mountedLibraryId, name, mimeType: text(item.mime_type) }
      : libraryId ? { libraryId } : id && !(previewUrl(item.cloud_doc_url) || previewUrl(record(item.extra).cloud_doc_url) || previewUrl(item.internalHref) || previewUrl(item.url))
        ? id.startsWith('file-') ? { fileId: id } : { libraryId: id } : undefined;
    return target ? [{ target, name }] : [];
  }).filter((item, index, all) => all.findIndex(other => JSON.stringify(other.target) === JSON.stringify(item.target)) === index);
  const excerpts = items.flatMap(item => {
    ...
    const pointer = Object.keys(record(item.input_pointer)).length ? record(item.input_pointer) : record(record(item.fff_metadata).input_pointer);
    ...
    const range = typeof page === 'number' ? `p. ${page}${item.page_range_end !== undefined && item.page_range_end !== page ? `–${item.page_range_end}` : ''}`
      : typeof line === 'number' ? `L${line}${lineEnd !== undefined ? `–${lineEnd}` : ''}` : '';
    return [range, snippet].filter(Boolean);
  });
  return { label, kind: 'source', links, files, excerpts: [...new Set(excerpts)], citationUuid: text(reference.citation_uuid) || undefined };
}
...
export function previewUrl(value: unknown): string | undefined {
  ...
  } catch { /* An invalid URL remains a non-interactive label. */ }
}
...
export function getPreviewContent(message: ConversationMessage, sourceLabel: string, imageLabel: string) {
  ...
  const attachments = [...records(message.metadata.attachments), ...records(message.metadata.mounted_library_file_references)].map(attachment => ({
    id: text(attachment.id) || text(attachment.file_id),
    libraryId: /^(?:file-library:)?external-/.test(text(attachment.library_file_id)) ? '' : text(attachment.library_file_id),
    mountedLibraryId: text(attachment.mounted_library_file_id) || (/^(?:file-library:)?external-/.test(text(attachment.library_file_id)) ? text(attachment.library_file_id) : ''),
    href: previewUrl(attachment.download_url),
    name: text(attachment.name), type: text(attachment.mime_type),
  })).filter((attachment, index, items) => attachment.name && items.findIndex(other =>
    attachment.id ? other.id === attachment.id : attachment.libraryId ? other.libraryId === attachment.libraryId
      : attachment.mountedLibraryId ? other.mountedLibraryId === attachment.mountedLibraryId : other.name === attachment.name) === index);
  type Reference = PreviewReferenceData;
  const references: Reference[] = [];
  const replacements: { start: number; end: number; index: number }[] = [];
  const addReference = (start: unknown, end: unknown, label: string, kind: Reference['kind'], links: Reference['links'] = []) => {
    if (typeof start !== 'number' || typeof end !== 'number' || !Number.isInteger(start) || !Number.isInteger(end) ||
        start < 0 || end <= start || end > source.length) return false;
  ...
  for (const reference of records(message.metadata.content_references)) {
    ...
    const metadata = records(message.metadata.conversation_context_citation_metadata).find(item => item.citation_uuid === reference.citation_uuid);
    const parsed = parsePreviewReference({ ...reference, ...record(metadata?.citation) }, sourceLabel);
  ...
  for (const metadata of records(message.metadata.conversation_context_citation_metadata)) {
    ...
    if (parsed.citationUuid && references.some(item => item.citationUuid === parsed.citationUuid)) continue;
  ...
  const images = records(message.content.parts).filter(part => part.content_type === 'image_asset_pointer').map(part => ({
    ...
    fileId: /^file-service:\/\//.test(text(part.asset_pointer)) ? text(part.asset_pointer).slice('file-service://'.length) : undefined,
    alt: imageLabel,
  }));
  return {
    markdown: normalizeMath(markdown), references,
    attachments, images,
  };
}
