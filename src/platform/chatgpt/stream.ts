...
export function createMessageStreamParser(onMessage: (value: unknown) => void, onControl: (value: Record<string, unknown>) => void) {
  let buffer = '';
  let operation = 'add';
  let path = '';
  let channel = 0;
  ...
  const apply = (root: unknown, op: string, pointer: string, value: unknown): unknown => {
    if (op === 'patch') {
      ...
      for (const patch of value) {
        if (!object(patch) || typeof patch.o !== 'string' || typeof patch.p !== 'string') throw new ConversationDataError('Invalid stream operation');
        root = apply(root, patch.o, patch.p, patch.v);
      }
      return root;
    }
    if (!['add', 'replace', 'append', 'remove'].includes(op)) throw new ConversationDataError('Unsupported stream operation');
    if (pointer === '') {
      if (op !== 'add' && op !== 'replace') throw new ConversationDataError('Invalid stream root operation');
      return value;
    }
    ...
    if (keys.some(key => ['__proto__', 'constructor', 'prototype'].includes(key))) throw new ConversationDataError('Unsafe stream path');
    let target = root as Record<string, unknown>;
    for (const key of keys.slice(0, -1)) {
      if (!target || typeof target !== 'object' || !Object.hasOwn(target, key)) throw new ConversationDataError('Missing stream path');
      target = target[key] as Record<string, unknown>;
    }
    if (!target || typeof target !== 'object') throw new ConversationDataError('Missing stream target');
    const key = keys.at(-1)!;
    if (op === 'append') {
      const previous = target[key];
      if (typeof previous === 'string' && typeof value === 'string') target[key] = previous + value;
      else if (Array.isArray(previous) && Array.isArray(value)) target[key] = [...previous, ...value];
...
export function installMessageStreamCapture(publisher: ReturnType<typeof createHistoryPublisher>) {
  ...
  function createSession(userId: string, conversationId: string | undefined, requestStartedAt: number, branchParentId: string) {
    ...
    const finish = (phase: 'complete' | 'interrupted') => {
      ...
      finished = true;
      emit(phase);
      if (conversationId && sessions.get(conversationId) === session) sessions.delete(conversationId);
    };
  ...
  WebSocket.prototype.send = function (data) {
    if (!watched.has(this)) {
      ...
      this.addEventListener('message', event => {
        ...
        let frames: unknown;
        try { frames = JSON.parse(event.data); } catch { return; }
        if (!Array.isArray(frames)) return;
        for (const frame of frames) {
          const catchups: unknown[] = Array.isArray(frame?.reply?.catchups) ? frame.reply.catchups : [];
          for (const candidate of [frame, ...catchups]) {
            ...
            try {
              ...
              else if (item.stream_item_id && item.encoded_item && !session.seen.has(item.stream_item_id)) {
  ...
  const readResponse = async (response: Response, session: Session) => {
    ...
    try {
      while (!session.finished) {
        ...
        if (chunk.done) break;
        if (publisher.isStopped()) { session.finish('interrupted'); break; }
  ...
  const captureRequest = async (input: RequestInfo | URL, init: RequestInit | undefined, response: Promise<Response>, userId: string) => {
  ...
  return {
    ...
    async captureResume(url: URL, response: Promise<Response>) {
      ...
      const session = [...sessions.entries()].find(([id, session]) => session.handedOff &&
        (url.pathname.includes(id) || [...session.topics].some(topic => url.pathname.includes(topic))))?.[1];
      if (!session) return;
      try {
        const result = await response;
      ...
      } catch { session.finish('interrupted'); }
    },
  };
}
