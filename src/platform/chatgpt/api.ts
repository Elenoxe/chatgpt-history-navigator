...
export async function fetchConversationSources(
      ...
      if (value.type === "conversation_context_source") {
        ...
        const index =
          typeof item.citation_uuid === "string"
            ? sources.findIndex((source) => source.citation_uuid === item.citation_uuid)
            : -1
        if (index < 0) sources.push(item)
        else sources[index] = { ...sources[index], ...item }
      }
      if (value.type === "error")
        throw new ConversationDataError("Conversation sources stream failed")
    }
  }
  try {
    while (true) {
